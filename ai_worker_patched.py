...
def _train_symbol_model(self, symbol, indicators, sentiment=None):
    """Deterministic confidence & accuracy based purely on technicals"""
    try:
        # Mapping indicator -> weight (importance)
        indicator_weights = {
            'rsi_14': 1.0,
            'macd_line': 1.0,
            'macd_signal': 0.8,
        ...
        for name, weight in indicator_weights.items():
            ...
            # Very simple bullish/bearish heuristic:
            #   Positive value -> bullish (1), otherwise bearish (0)
            #   For RSI use >50 bullish
            if name.startswith('rsi'):
                bullish = 1.0 if value > 50 else 0.0
            else:
                bullish = 1.0 if value > 0 else 0.0

            scores.append(bullish * weight)
            weights.append(weight)

        if not scores:
            return 65.0, 65.0  # default if no data

        raw_confidence = sum(scores) / sum(weights)
        confidence = raw_confidence * 100  # scale to percentage

        # Volatility penalty using ATR vs EMA (if present)
        atr = indicators.get('atr')
        ema = indicators.get('price_ema_12') or 1.0
        if atr is not None and ema:
            vol_penalty = max(0.75, 1 - min(1, atr / ema))
        ...
        # Fallback to thresholds from DB
        try:
            db_settings = self.database.load_settings()
            thr = float(db_settings.get('accuracyThreshold', 70))
            return thr, thr
        except Exception:
            return 70.0, 70.0
