...
class AIWorker:
    ...
    def _calculate_technical_indicators(self, symbol, market_data):
        """Extended technical indicators for the symbol"""
        ...
        try:
            ...
            atr14 = np.mean(trs[-14:]) if len(trs) >= 14 else np.mean(trs)
            ...
            self.console_logger.log('WARNING', f'Failed to calc indicators for {symbol}: {e}')
            return {}
...
def _train_symbol_model(self, symbol, indicators, sentiment=None):
    """Deterministic confidence / accuracy from extended indicator set"""
    try:
        weights = {
            'rsi_14': 1.0,
            'stoch_k': 0.9,
        ...
        for name, w in weights.items():
            ...
            if val is None:
                continue
            bullish = 0.5  # neutral default

            if name in ('rsi_14', 'stoch_k'):
                bullish = 1.0 if val >= 50 else 0.0
            elif name == 'williams_r':
                bullish = 1.0 if val >= -50 else 0.0
            elif name in ('cci', 'macd_line', 'macd_signal', 'obv_change'):
                bullish = 1.0 if val > 0 else 0.0
            elif name == 'donch_width':
                # narrow channel (<2%) -> likely breakout soon, neither bull nor bear
                bullish = 0.5
            else:  # sma/ema price relations etc.
                bullish = 1.0 if val > 0 else 0.0
        ...
        confidence = (score / total_w) * 100

        # Volatility penalty
        atr = indicators.get('atr')
        ema = indicators.get('price_ema_12') or 1.0
        if atr is not None and ema:
            confidence *= max(0.7, 1 - min(1, atr / ema))
        ...
        self.console_logger.log('WARNING', f'Confidence calc failed for {symbol}: {e}')
        try:
            thr = float(self.database.load_settings().get('accuracyThreshold', 70))
            return thr, thr
        except Exception:
            return 70.0, 70.0
